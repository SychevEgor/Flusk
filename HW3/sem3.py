...
from models import db, Student, Faq, GenderEnum, Author, Book, Estimate, User, UserHW
from random import choice
from flask_wtf.csrf import CSRFProtect
from forms import RegistrationForm, Registration2Form, RegistrationHWForm
...
category = [
    {"title": 'Базовая страница', "func_name": 'index'},
    {"title": 'Студенты', "func_name": 'get_all'},
    {"title": 'Книги', "func_name": 'get_all_books'},
    {"title": 'ОЦенки', "func_name": 'get_all_est'},
    {"title": 'Регистрация', "func_name": 'registration'},
    {"title": 'Регистрация 2', "func_name": 'registration2'},
    {"title": 'Регистрация ДЗ', "func_name": 'registration3'}
]
...
def registration3():
    form = RegistrationHWForm()
    if request.method == 'POST' and form.validate():

        name = form.name.data.lower()
        surname = form.surname.data.lower()
        email = form.email.data
        user = UserHW(name=name, surname=surname, email=email)
        if UserHW.query.filter(UserHW.email == email).first():
    ...
    return render_template('registrationHW.html', form=form)
